...
class TabbrVC: UITabBarController {
    ...
    override func viewDidLoad() {
        ...
        let item1 = UITabBarItem.init(title: NSStringFromClass(cardVC.classForCoder), image: nil, selectedImage: nil);
        navi1.tabBarItem = item1;
        
        let remindVC = RemindVC.init(nibName: nil, bundle: nil);
        remindVC.title = NSStringFromClass(remindVC.classForCoder);
        var navi2 = UINavigationController.init(rootViewController: remindVC);
        let item2 = UITabBarItem.init(title: NSStringFromClass(remindVC.classForCoder), image: nil, selectedImage: nil);
        navi2.tabBarItem = item2;

        
        let mapVC  = MapVC.init(nibName: nil, bundle: nil);
        mapVC.title = NSStringFromClass(mapVC.classForCoder);
        var navi3 = UINavigationController.init(rootViewController: mapVC);
        let item3 = UITabBarItem.init(title: NSStringFromClass(mapVC.classForCoder), image: nil, selectedImage: nil);
        navi3.tabBarItem = item3;
        
        let settingVC = SettingVC.init(nibName: nil, bundle: nil);
        settingVC.title = NSStringFromClass(settingVC.classForCoder);
        var navi4 = UINavigationController.init(rootViewController: settingVC);
        let item4 = UITabBarItem.init(title: NSStringFromClass(settingVC.classForCoder), image: nil, selectedImage: nil);
